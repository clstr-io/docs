...
Run `clstr list` to see all available challenges. Start with the [distributed key-value store challenge](/kv-store/).

### Scaffolding

Run `clstr init kv-store --language go` to create a new challenge directory with:
...
When you pass, run `clstr next` to unlock the next stage. Each stage builds on the previous one, gradually adding complexity. CI runs `clstr test --so-far` on every push, so your implementation must continue satisfying all earlier stages as you advance.
