...
Name tests as declarative subject-verb statements that read like assertions: "Leader Election Completes", "Partition Enforces Quorum", "Followers Redirect to Leader". The name should state what must be true, so a failure is immediately informative without reading the test body.
