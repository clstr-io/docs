...
A `NodeSelector` controls which nodes receive a request and how the assertion is evaluated.

### `Node(name)`

Targets a single named node. The assertion passes if that node satisfies all conditions.
...
Passes when every node satisfies the assertion. With no arguments, targets all nodes in the cluster. Pass names to target a subset:
...
Passes when exactly one node satisfies the assertion. Useful for asserting a single leader:
...
Passes when at least one node satisfies the assertion:
...
Makes a one-off GET request to a named node and returns a `*FetchResponse`. Returns `nil` if the node is unreachable. Useful for reading state before an assertion:
