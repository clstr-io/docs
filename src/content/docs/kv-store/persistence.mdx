...
1. Check the `--data-dir` path for existing data
...
Save your in-memory state to disk during shutdown and restore it on startup. Create your data files in the data directory (passed via `--data-dir`). The serialization format and file naming are up to you - JSON, binary ([Protocol Buffers](https://protobuf.dev/), [MessagePack](https://msgpack.org/index.html), [BSON](https://bsonspec.org/), [gob](https://pkg.go.dev/encoding/gob), [pickle](https://docs.python.org/3/library/pickle.html)), plain text, whatever.
...
The test harness mounts a persistent volume at `/app/data` and passes `--data-dir=/app/data` to your server. Your server should store and load data from the path given by `--data-dir`.
