...
The test harness mounts a persistent volume at `/app/data` and passes `--data-dir=/app/data`, same as the previous stage. Your server should store and load data from the path given by `--data-dir`.
